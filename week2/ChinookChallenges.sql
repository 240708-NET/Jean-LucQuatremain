...
SELECT FirstName + ' ' + LastName AS full_name, CustomerId
    FROM [MyDatabase].[dbo].[Customer]
...
SELECT COUNT(InvoiceId) AS TotalInvoices, BillingCountry
    FROM [MyDatabase].[dbo].[Invoice]
    GROUP BY BillingCountry;

-- Retrieve the total sales per country, ordered by the highest total sales first.
SELECT SUM(Total) AS TotalSum, BillingCountry
    FROM [MyDatabase].[dbo].[Invoice]
    GROUP BY BillingCountry
    ORDER BY TotalSum DESC, BillingCountry;
